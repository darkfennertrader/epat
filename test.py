...
def backtest_strategy(data):
    ...
    print(f'"Buy and hold" Strategy logreturns: {logreturns} converted to simple returns: {(np.exp(logreturns) - 1):.2f}')
    
    #logreturns = np.round((data['buy_and_sell_returns']).cumsum()[-1], 2)
    
    logreturns = np.round((data['bbands_returns']).cumsum()[-1], 2)
    print(f'"Bollinger Bands" Strategy logreturns: {logreturns} converted to simple returns: {(np.exp(logreturns) - 1):.2f}')
